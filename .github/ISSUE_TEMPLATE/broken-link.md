...
- fix/replace the `repository` URL for the affected entry in `data/modules.yaml`
  and regenerate the README (`python3 scripts/generate_readme.py`), or
- remove the entry if the project is no longer available, or
- close this issue if the failure was a false positive (e.g. the site blocks
  bots) and add it to `.markdown-link-check.json`'s `ignorePatterns` if it'll
  keep recurring.
